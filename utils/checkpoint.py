...
class Checkpointer(object):
    ...
    def load(self, load_name=""):
        if load_name:
            f = os.path.join(self.save_dir, load_name)
        else:
            f = self.get_last_checkpoint_file()
        if not f:
            # no checkpoint selected and no latest checkpoint fil
    ...
    def get_last_checkpoint_file(self):
        ...
        try:
            ...
            # if file doesn't exist, maybe because it has been deleted by a separate process
            last_saved = ""
        return last_saved
